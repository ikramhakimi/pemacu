...
$page_title   = 'Orders - Unpaid Orders';
...
$unpaid_orders = array_values(array_filter($orders, static function (array $order_item): bool {
  return isset($order_item['payment_status']) && $order_item['payment_status'] === 'Unpaid';
}));

$current_page = isset($_GET['page']) ? max(1, (int) $_GET['page']) : 1;
$per_page     = 10;
$total_items  = count($unpaid_orders);
$total_pages  = max(1, (int) ceil($total_items / $per_page));
$current_page = min($current_page, $total_pages);
$offset       = ($current_page - 1) * $per_page;
$page_orders  = array_slice($unpaid_orders, $offset, $per_page);
...
$dashboard_breadcrumb_items = [
  ...
  ['label' => 'Unpaid Orders', 'current' => true],
];
$dashboard_breadcrumb_description = 'Review and manage unpaid orders, including payment progress and order status.';
...
<header class="app-header py-6 px-4 -mx-4 md:hidden">
  <div>
    <h1 class="text-3xl font-semibold leading-none text-brand-900">Unpaid Orders</h1>
    <p class="mt-4 max-w-2xl text-brand-600">
      Review and manage unpaid orders, including payment progress and order status.
    </p>
  </div>
</header>
...
<article class="app-article pb-20 space-y-6 pt-6">
  ...
  <?php component('pagination', [
    'pagination' => [
      ...
      'show_info'    => true,
      'base_url'     => path('/dashboard/orders/unpaid-orders'),
      'page_param'   => 'page',
      'query'        => [],
    ],
  ]); ?>
