...
?>
<section class="rounded-lg border border-brand-200 bg-white p-5" aria-labelledby="report-criteria-breakdown-heading">
  <header class="border-b border-brand-200 pb-4">
    <h2 id="report-criteria-breakdown-heading" class="text-lg font-semibold text-brand-900">Criteria Breakdown</h2>
    <p class="mt-1 text-sm text-brand-600">Target, verified, potential, and risk position across GBI NRNC criteria.</p>
  </header>
  ...
  <div class="mt-4 space-y-3">
    <?php foreach ($criteria_rows as $criteria_row): ?>
      ...
      ?>
      <article class="rounded-md border border-brand-200 bg-brand-50 p-4">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div>
            <p class="font-medium text-brand-900"><?= e($code . ' - ' . $label); ?></p>
            <p class="mt-1 text-xs text-brand-600">Target <?= e($target_points); ?> pts | Verified <?= e($verified_points); ?> pts | Potential <?= e($potential_points); ?> pts | At Risk <?= e($risk_points); ?> pts</p>
          </div>
          <?php component('badge', ['items' => [['label' => $status, 'tone' => $status_tone]]]); ?>
        </div>
        <div class="mt-3 h-2 overflow-hidden rounded-full bg-brand-200" role="img" aria-label="<?= e($code); ?> progress">
