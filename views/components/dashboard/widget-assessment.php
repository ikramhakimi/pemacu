...
?>
<article class="widget widget--assessment state--<?= e($widget_state); ?> overflow-hidden rounded-lg bg-brand-800 ring-1-ring-brand-900 p-1">
  <div class="widget__head flex items-stretch gap-4 p-4 pr-1 bg-white rounded-md ring-1 ring-brand-600">
    ...
    <div class="widget__content">
      <h3 class="widget__title text-base font-medium text-brand-900"><?= e($widget_title); ?></h3>
      <?php if ($widget_description !== ''): ?>
        <p class="widget__description mt-1 text-brand-500"><?= e($widget_description); ?></p>
      <?php endif; ?>
    </div>
  </div>
  <div class="widget__foot flex items-center gap-4 px-4 pt-3 pb-2">
    <div class="widget__score w-16 shrink-0 text-center text-brand-200"><?= e($widget_score_display); ?> / <?= e($widget_max_display); ?></div>
    <div class="widget__progress h-3 w-full rounded-sm bg-brand-600" role="progressbar" aria-label="<?= e($widget_title); ?> score" aria-valuemin="0" aria-valuemax="<?= e($widget_max_display); ?>" aria-valuenow="<?= e($widget_score_display); ?>">
