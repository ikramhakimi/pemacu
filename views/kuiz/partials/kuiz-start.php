...
<body class="dashboard-ui bg-brand-100 text-sm text-brand-700 font-sans leading-relaxed tracking-[-2%]">
  <main id="root">
    <div class="<?= e('w-full min-h-screen'); ?>">
      <?php if (!$dashboard_no_sidebar): ?>
        <div
          id="dashboard-sidebar-panel"
          class="app-sidebar bg-brand-900 fixed inset-y-0 left-0 z-30 w-[240px] -translate-x-full overflow-y-auto border-r border-brand-200 bg-brand-100 px-2 py-5 transition-transform duration-200 ease-out xl:translate-x-0 js-dashboard-sidebar-panel"
          ...
          <div class="">
            ...
            <ul class="">
              ...
              <?php foreach ($dashboard_sidebar as $item_index => $item): ?>
                ...
                if ($section_label !== ''):
                ?>
                  <li class="pt-4 mt-4 mx-2 pb-2 px-1 text-xs font-medium uppercase tracking-wide text-brand-500 border-t border-brand-800">
                    <?= e($section_label); ?>
                  </li>
                  <?php
                  continue;
                ...
                <li class="group/sidebar-item js-dashboard-sidebar-item" data-dashboard-sidebar-item="<?= e($item_id_slug); ?>">
                  <a
                    class="font-medium flex items-center gap-3 rounded-md px-3 py-2 leading-5 text-brand-100 hover:bg-brand-700/30 hover:text-brand-50 js-dashboard-sidebar-parent-link"
                    href="<?= e($item_href); ?>"
                  ...
                  <?php if ($item_has_children): ?>
                    ...
                    >
                      <?php foreach ($item_children as $child_item): ?>
                        ...
                        <li>
                          <a
                            class="block rounded-md px-3 py-1 leading-5 text-brand-400 hover:bg-brand-800/50 hover:text-brand-50 js-dashboard-sidebar-child-link"
                            href="<?= e($child_href); ?>"
                          >
                            <?= e($child_label); ?>
                          </a>
