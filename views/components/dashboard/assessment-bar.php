...
$assessment_bar_base_class = 'assessment-bar rounded-lg border border-brand-200 bg-white transition hover:border-brand-400 hover:shadow-sm ring-0 ring-transparent hover:ring-2 hover:ring-brand-300';
$assessment_bar_container_class = trim($assessment_bar_base_class . ' ' . $assessment_bar_class);
?>
<article class="<?= e($assessment_bar_container_class); ?>">
  <div class="flex flex-wrap items-center gap-3 pl-5 py-3 pr-2">
    ...
    <h3 class="assessment-bar__title text-sm font-medium text-brand-900 sm:text-base">
      <?= e($assessment_bar_title); ?>
      <?php if ($assessment_bar_score !== ''): ?>
        <span class="assessment-bar__score font-mono text-xs text-brand-500 ml-2"><?= e($assessment_bar_score); ?></span>
      <?php endif; ?>
    </h3>

    <?php if ($assessment_bar_file_label !== '' || $assessment_bar_has_remarks): ?>
      <div class="assessment-bar__meta ml-auto flex items-center gap-4">
        ...
        <?php component('icon', [
          ...
          'icon_size'   => '24',
          'icon_class'  => 'assessment-bar__chevron text-current opacity-60 inline-block transition-transform duration-200 js-assessment-group-icon ml-auto',
          'icon_stroke' => '1',
        ]); ?>
      </div>
    <?php else: ?>
